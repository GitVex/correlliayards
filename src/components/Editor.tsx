...
type GroupKey = 'identity' | 'artwork' | 'arcs' | 'command' | 'defense' | 'armament' | 'speed' | 'slots'
...
export function Editor({
  ...
  // Listed in the order the panels appear, so this reads as the editor's outline.
  ...
  // Group counters, all read off the data rather than typed in, so they can't go
  ...
  const armedArcs = [
    ...
    cardData.armamentLeft,
    cardData.armamentRight,
    cardData.armamentRear,
  ].filter((dice) => parseDiceRows(dice).some((row) => row.length > 0)).length
  const defenseTokens = cardData.defenseTokens.filter((token) => token !== '—').length
  ...
  return (
    <section className="editor" aria-label="Card definition">
      ...
      <div
        ...
        role="tabpanel"
        aria-labelledby={tab === 'Fields' ? fieldsTabId : jsonTabId}
      >
        {tab === 'JSON' ? (
          ...
          <>
            ...
            <Group
              title="Speed chart"
              count={`${speedCount(cardData)} speeds`}
            ...
            <Group
              title="Upgrade slots"
              count={`${cardData.upgrades.length} slots`}
